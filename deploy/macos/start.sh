...
# 4H forward-test cohort (AO candidates with a 4H edge but <15 backtest trades).
# Free symbols (not in AUDITED_CONFIGS) so trades.db attribution is clean.
# Track to >=15 live trades with research/forward_test_report.py, then re-audit.
FORWARD_TEST_CONFIGS="config_zenusdt_awesome4h.json config_dashusdt_awesome4h.json config_aliceusdt_awesome4h.json config_tonusdt_awesome4h.json config_penguusdt_awesome4h.json"

exec "$PYTHON" main_multi.py --configs $AUDITED_CONFIGS $FORWARD_TEST_CONFIGS --max-positions 10 --risk 0.01 --leverage 25
