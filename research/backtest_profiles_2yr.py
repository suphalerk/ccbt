"""backtest_profiles_2yr.py — 5 deployment profiles with FULL 2-year data.

Key difference from backtest_5_profiles.py:
  - Uses ALL available data (no filter_last_year / iloc[-8760:] truncation)
  - Updated top5_v2 bot set per spec
  - Output to data/profiles_2yr.json

Approach:
  1. Build the audited bot list (same structure as backtest_5_profiles.py).
  2. Run BacktestEngine ONCE per bot → collect R-multiple trade pool from full data.
  3. Replay the shared wallet 5 times with different capital/risk/leverage/bot-set settings.

Profiles:
  testnet   — $5,000  | 1.0% risk | 25x lev | all bots   | max 10 concurrent
  real-test — $45     | 3.0% risk | 10x lev | top5_v2    | max 2 concurrent
  real-safe — $200    | 1.0% risk | 10x lev | top 10     | max 5 concurrent
  real-grow — $500    | 0.9% risk | 15x lev | top 50     | max 10 concurrent
  real-full — $1,000  | 0.9% risk | 25x lev | all bots   | max 10 concurrent
...
Output: stdout + data/profiles_2yr.json
"""
...
DATA_DIR = ROOT / "data"
MIN_DATA_MONTHS = 6   # minimum span required; full available data is used (no 1yr cap)
MIN_TRADES = 6
...
# ---------------------------------------------------------------------------
# top5_v2: hardcoded per spec for "real-test" profile
# ---------------------------------------------------------------------------
# Format: (label, coin, prefix, signal_key, tf, sl, tp, trail, risk_pct)
TOP_5_V2_DEFS = [
    ("DASH Ichi4H v2",       "DASH",     "dashusdt",     "ichimoku_cloud", "4h", 1.0, 2.0, 2.0, 0.01),
    ("BTC EMA 15m v2",       "BTC",      "btcusdt",      "ema_crossover",  "15m", 1.0, 3.0, 2.0, 0.05),
    ("XAUUSD ribbon_ao v2",  "XAUUSD",   "xauusdt",      "ribbon_ao",      "4h", 2.0, 4.0, 3.0, 0.01),
    ("AVAX EMAribbon 4H v2", "AVAX",     "avaxusdt",     "ema_ribbon",     "4h", 2.5, 5.0, 4.0, 0.02),
    ("1000SHIB EMArib 1H v2","1000SHIB", "1000shibusdt", "ema_ribbon",     "1h", 2.0, 4.0, 3.0, 0.01),
]

TOP_5_V2_LABELS: set[str] = {row[0] for row in TOP_5_V2_DEFS}

# ---------------------------------------------------------------------------
# Removed bots (failed audit — same list as backtest_5_profiles.py)
# ---------------------------------------------------------------------------
REMOVE_BOTS: set[str] = {
    "ADA awesome_ R10",
    "ADA zscore_m R11",
...
# ---------------------------------------------------------------------------
# Data helpers — FULL data, no year-filtering
# ---------------------------------------------------------------------------
...
def load_signal_data(prefix: str, tf: str) -> pd.DataFrame:
    """Load full available data — no year cap."""
    if tf == "15m":
...
def load_trend_data(prefix: str, tf: str) -> pd.DataFrame:
    """Load 1h trend data for 15m signal bots — full data."""
    if tf != "15m":
        return pd.DataFrame()
    for suffix in ("_1h_2y.csv", "_1h_5y.csv"):
        path = DATA_DIR / f"{prefix}{suffix}"
...
# ---------------------------------------------------------------------------
# Bot registry — same as backtest_5_profiles.py + top5_v2 bots
# ---------------------------------------------------------------------------
...
def _build_bot_list() -> list[BotDef]:
    ...
    # Add top5_v2 bots (for real-test profile)
    top5v2_seen: set[str] = set(f"{b.prefix}_{b.signal_key}_{b.tf}" for b in deduped)
    for label, coin, prefix, signal, tf, sl, tp, trail, risk in TOP_5_V2_DEFS:
        key = f"{prefix}_{signal}_{tf}"
        if key not in top5v2_seen:
            deduped.append(BotDef(
                label=label, coin=coin, prefix=prefix, signal_key=signal,
                tf=tf, sl=sl, tp=tp, trail=trail, risk_pct=risk,
            ))
...
# ---------------------------------------------------------------------------
# Per-bot engine runner — FULL data (no filter_last_year)
# ---------------------------------------------------------------------------

def run_bot_engine(bot: BotDef) -> list[dict]:
    """Run BacktestEngine for one bot on ALL available data. Returns trade records with r_mult."""
    ...
    # Use FULL data — no year truncation
    if len(signal_df) < 50:
        return []
...
def main() -> None:
    ...
    print(sep)
    print("5 DEPLOYMENT PROFILES  —  FULL 2-YEAR DATA  (vs 1yr in backtest_5_profiles.py)")
    print(sep)
    print()
    ...
    # ------------------------------------------------------------------
    print("Step 1: Building audited bot list...")
    ...
    print(f"  Full list: {len(all_bots)} bots | Removed {len(removed)} FAIL bots | {len(bots)} active")

    unmatched = REMOVE_BOTS - {b.label for b in all_bots}
    if unmatched:
        print(f"  NOTE: {len(unmatched)} REMOVE labels not matched (likely already excluded):")
        for u in sorted(unmatched):
            print(f"    - {u!r}")

    # ------------------------------------------------------------------
    # Step 2: Run BacktestEngine once per bot on FULL data
    # ------------------------------------------------------------------
    print()
    print("Step 2: Running BacktestEngine (FULL data — no 1yr cap)...")
    print()
    ...
    # Sort active bots by total R (proxy for PnL contribution over full period)
    sorted_by_r = sorted(
        all_active_labels,
    ...
    # top_5: resolve top5_v2 spec — match by (prefix, signal_key, tf) to canonical label
    # because some v2 labels differ from the registered label for the same strategy
    key_to_active_label: dict[str, str] = {}
    for b in bots:
        if b.label not in skipped_bots:
            k = f"{b.prefix}_{b.signal_key}_{b.tf}"
            if k not in key_to_active_label:
                key_to_active_label[k] = b.label

    top_5_set: set[str] = set()
    for label, coin, prefix, signal, tf, _sl, _tp, _trail, _risk in TOP_5_V2_DEFS:
        k = f"{prefix}_{signal}_{tf}"
        if label in all_active_labels:
            top_5_set.add(label)
        elif k in key_to_active_label:
            resolved = key_to_active_label[k]
            top_5_set.add(resolved)
            print(f"  top5_v2: {label!r} -> resolved to {resolved!r}")
        else:
            print(f"  WARNING: top5_v2 bot {label!r} not found (key={k}), will supplement")

    # Fill any gaps with top-R bots
    if len(top_5_set) < len(TOP_5_V2_DEFS):
        supplement = [b for b in sorted_by_r if b not in top_5_set]
        needed = len(TOP_5_V2_DEFS) - len(top_5_set)
        top_5_set.update(supplement[:needed])
        print(f"  top5_v2: supplemented {needed} missing bot(s) with top-R bots")

    top_10_set = set(sorted_by_r[:10])
    top_50_set = set(sorted_by_r[:50])

    print(f"  all_bots: {len(all_active_labels)} active bots")
    print(f"  top_50:   {len(top_50_set)} bots  (top by total R over 2yr)")
    ...
    # ------------------------------------------------------------------
    print(sep)
    print("5 DEPLOYMENT PROFILES COMPARISON  —  FULL 2-YEAR DATA")
    print(sep)
    print()
    ...
    for pr in profile_results:
        ...
        print(f"[{prof['name']}] Monthly breakdown ({len(monthly)} months, {pr['n_months']} months total):")
    ...
    # ------------------------------------------------------------------
    # Step 8: Data span info per bot set
    # ------------------------------------------------------------------
    print(sep)
    print("DATA SPAN SUMMARY (sample of bots)")
    print(sep)
    sample_labels = list(all_active_labels)[:10]
    for bot in bots:
        if bot.label not in sample_labels:
            continue
        signal_df = load_signal_data(bot.prefix, bot.tf)
        months = data_span_months(signal_df) if not signal_df.empty else 0.0
        print(f"  {bot.label:<35} {months:.1f} months of data")
    print()
    ...
    output = {
        "meta": {
            "description": "5 deployment profiles, audited bots, FULL 2-year data (no 1yr cap)",
            "engine_balance": ENGINE_BALANCE,
            "total_trade_pool": len(all_trade_records),
            "active_bots": active_bots,
            "data_note": "Uses all available CSV data — no filter_last_year applied",
        },
        "profiles": [],
    }
    ...
    out_path = DATA_DIR / "profiles_2yr.json"
