"""r13_combined_backtest.py — Full combined portfolio: 125 existing bots + 11 R13 bots.

Sources:
  - Source 1: All 125 existing bots from final_backtest_fixed.json → portfolio_summary.per_bot
              Rebuilt via the same _build_bot_list() approach as final_backtest.py
  - Source 2: 11 new R13 bots from pipeline_20260323_104924.json
              Filter: PF >= 1.3 AND trades >= 15

R13 Bots (11 deploy-grade from audited_bots):
  1000PEPE  dualthrust_adx 1h   sl=2.0 tp=4.0 trail=3.0
  WLD       dualthrust_adx 1h   sl=2.0 tp=4.0 trail=3.0
  ARB       dualthrust_adx 1h   sl=2.0 tp=4.0 trail=3.0
  XAUUSD    ribbon_ao      4h   sl=2.0 tp=4.0 trail=3.0
  ICP       ribbon_ao      4h   sl=2.0 tp=4.0 trail=3.0
  XLM       ribbon_ao      4h   sl=2.0 tp=4.0 trail=3.0
  AAVE      dualthrust_adx 1h   sl=2.0 tp=4.0 trail=3.0
  HBAR      ribbon_ao      1h   sl=2.0 tp=4.0 trail=3.0
  BAN       ribbon_ao      1h   sl=2.0 tp=4.0 trail=3.0
  XAI       ribbon_rsi_vol 4h   sl=2.0 tp=4.0 trail=3.0
  IP        ribbon_rsi_vol 4h   sl=2.0 tp=4.0 trail=3.0

R-multiple fix:
  Each bot's engine runs with $10K virtual balance.
  R-multiple = trade.pnl / (10_000 * actual_risk_pct)
  Shared wallet always applies 1% risk: dollar_pnl = balance * 0.01 * r_multiple
  BTC=5%, WIF=3% config risk → larger R-multiples reflect actual alpha, 1% applied in shared wallet.
...
Reports:
  ...
  2. Monthly returns with per-bot breakdown (all months)
  3. Daily log (last 2 months)

Output: stdout + data/r13_combined.json
"""
...
# ---------------------------------------------------------------------------
# Coins whose ALL existing bots get replaced by upgrade versions (from final_backtest.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# R13 bots — 11 deploy-grade from pipeline_20260323_104924.json
# Loaded dynamically from the pipeline file, but also hardcoded here as fallback.
# Override SL/TP/trail per user spec (use spec values regardless of pipeline values).
# ---------------------------------------------------------------------------
R13_SPEC = [
    # (coin, signal, tf, sl, tp, trail)  — per user specification
...
# ---------------------------------------------------------------------------
# Bot registry — mirrors final_backtest.py + adds R13 bots
# ---------------------------------------------------------------------------
...
def _build_bot_list() -> list[BotDef]:
    """Build unified bot list: all rounds from final_backtest.py sources + R13 bots."""
    bots: list[BotDef] = []

    # -------------------------------------------------------------------
    # EXISTING BOTS (prior to R10 — matches final_backtest.py)
    # -------------------------------------------------------------------
    existing_defs = [
    ...
    # -------------------------------------------------------------------
    # Add R13 bots from pipeline file (PF >= 1.3, trades >= 15)
    # Use user-specified SL/TP/trail (not pipeline values)
    # -------------------------------------------------------------------
    r13_added: set[str] = set()
    # Build lookup: (coin.upper(), signal, tf) -> spec
    r13_spec_map = {
    ...
    if pipeline_path.exists():
        ...
        audited_bots = pipeline_data.get("audited_bots", [])
        for ab in audited_bots:
...
def run_bot_engine(bot: BotDef) -> tuple[list, dict]:
    ...
    # CRITICAL: use ACTUAL risk from the config the engine will use
    # This ensures R-multiple is correctly normalised regardless of config_file overrides.
    ...
    def close_expired(current_time: pd.Timestamp) -> None:
        ...
        for ot in open_trades:
            if ot["exit_time"] <= current_time:
                # Shared wallet ALWAYS 1% risk applied to R-multiple.
                # R-multiple was computed with ACTUAL engine risk (bot.risk_pct),
                # so bots with higher configured risk (BTC=5%, WIF=3%) get larger R-multiples
                # but we apply flat 1% here for fair shared-wallet contribution.
...
def report_per_bot(per_bot_stats: dict, bot_metrics: list[dict], r13_labels: set[str]) -> str:
    """Report 1: per-bot summary table sorted by shared wallet PnL$."""
    lines = [
        "",
        "=" * 100,
        "REPORT 1: PER-BOT SUMMARY (sorted by shared wallet PnL$)  [*] = R13 new bot",
        "=" * 100,
        f"{'Bot':<36} {'Strategy':<22} {'Trades':>6}  {'WR%':>5}  {'Avg R':>6}  {'PnL$':>9}",
        "-" * 90,
    ]
    ...
    for bot_name, stats in per_bot_stats.items():
        ...
        tf = eng_m.get("tf", "?")
        strat = f"{signal[:16]} {tf}"
        is_r13 = bot_name in r13_labels
        rows.append((bot_name, strat, n, wr, avg_r, pnl, is_r13))

    rows.sort(key=lambda x: -x[5])
    total_pnl = 0.0
    total_trades = 0
    for bot_name, strat, n, wr, avg_r, pnl, is_r13 in rows:
        mark = " (*)" if is_r13 else "     "
        lines.append(
            f"  {bot_name + mark:<36} {strat:<22} {n:>6}  {wr:>4.0f}%  {avg_r:>+6.3f}  {pnl:>+9.2f}"
        )
        total_pnl += pnl
        total_trades += n
    ...
    lines += [
        "-" * 90,
        f"  {'TOTAL':<36} {'':<22} {total_trades:>6}  {'':>5}  {'':>6}  {total_pnl:>+9.2f}",
        "=" * 100,
    ]
    return "\n".join(lines)
...
def report_monthly(monthly_rows: list[dict]) -> str:
    """Report 2: monthly returns with per-bot breakdown (all months)."""
    lines = [
        "",
        "=" * 65,
    ...
    for row in monthly_rows:
        ...
        for b in row["bots"]:
            lines.append(
                f"    {b['bot']:<36}  {b['trades']:>3} trades  {b['pnl']:>+9.2f}"
            )
        lines.append(f"  {'MONTH TOTAL':<36}  {row['trades']:>3} trades  {row['pnl']:>+9.2f}")
    lines.append("\n" + "=" * 65)
    return "\n".join(lines)
...
def report_daily(daily_log: list[dict]) -> str:
    ...
    lines = [
        ...
        "=" * 105,
        f"{'Date':<12}  {'Bot':<36}  {'Side':<6}  {'R':>7}  {'PnL$':>9}  {'Balance$':>10}",
        "-" * 90,
    ]
    for _, row in recent.iterrows():
        lines.append(
            f"{row['date']:<12}  {row['bot']:<36}  {row['side']:<6}  "
...
def main() -> None:
    print("=" * 90)
    print("R13 COMBINED BACKTEST — 125 EXISTING BOTS + 11 NEW R13 BOTS")
    print(f"Shared wallet: ${INITIAL_WALLET:.0f}  |  Max concurrent: {MAX_CONCURRENT}  |  Min trades: {MIN_TRADES}")
    print(f"R-multiple: ACTUAL engine risk per bot  |  Shared wallet: always 1%")
    print("=" * 90)

    # Build unified bot list
    bots = _build_bot_list()
    r13_bots = [b for b in bots if b.is_r13]
    existing_bots = [b for b in bots if not b.is_r13]
    upgrade_labels = {u[0] for u in UPGRADES}

    print(f"\nBuilt {len(bots)} unique bot definitions:")
    print(f"  - {len(existing_bots)} existing bots (incl. {sum(1 for b in existing_bots if b.label in upgrade_labels)} upgrades)")
    print(f"  - {len(r13_bots)} new R13 bots: {[b.label for b in r13_bots]}")

    # Run each bot through the engine
    all_trade_records: list[dict] = []
    bot_metrics: list[dict] = []
    skipped: list[str] = []
    r13_labels: set[str] = set()

    print(f"\nRunning backtest engines ({len(bots)} bots)...")
    for i, bot in enumerate(bots):
        ...
        # CRITICAL: Use ACTUAL risk from config (not bot.risk_pct which may differ for config_file bots).
        # This ensures BTC (5% risk) and WIF (3% risk) get larger R-multiples, while
        # the shared wallet replay normalises by applying flat 1%.
        actual_risk_pct = metrics["actual_risk_pct"]
        risk_amount = 10_000.0 * actual_risk_pct

        if bot.is_r13:
            r13_labels.add(bot.label)
    ...
    # R13 results summary
    r13_metrics = [m for m in bot_metrics if m.get("is_r13")]
    if r13_metrics:
        print(f"\nR13 engine results ({len(r13_metrics)} bots):")
        print(f"  {'Bot':<36} {'Signal':<18} {'Tr':>4}  {'WR%':>5}  {'PF':>5}  {'AvgR':>6}  {'PnL$10K':>9}")
        print("  " + "-" * 85)
        for m in sorted(r13_metrics, key=lambda x: -x["pnl_10k"]):
            pf_str = f"{m['pf']:.2f}" if m["pf"] < 99 else "inf"
            print(
                f"  {m['label']:<36} {m['signal'][:16]:<16} {m['tf']:<4} "
                f"{m['trades']:>4}  {m['wr_pct']:>4.0f}%  {pf_str:>5}  "
                f"{m['avg_r']:>+6.3f}  {m['pnl_10k']:>+9.2f}"
            )

    # Full engine metrics
    print("\nEngine metrics (all bots, independent $10K runs, sorted by PnL$):")
    print(f"  {'Bot':<36} {'Signal':<18} {'Tr':>4}  {'WR%':>5}  {'PF':>5}  {'AvgR':>6}  {'PnL$10K':>9}")
    print("  " + "-" * 85)
    for m in sorted(bot_metrics, key=lambda x: -x["pnl_10k"]):
        pf_str = f"{m['pf']:.2f}" if m["pf"] < 99 else "inf"
        r13_mark = " (*)" if m.get("is_r13") else "     "
        print(
            f"  {m['label'] + r13_mark:<36} {m['signal'][:16]:<16} {m['tf']:<4} "
            f"{m['trades']:>4}  {m['wr_pct']:>4.0f}%  {pf_str:>5}  "
            f"{m['avg_r']:>+6.3f}  {m['pnl_10k']:>+9.2f}"
        )
    ...
    # Comparison vs previous (final_backtest_fixed.json)
    prev_path = DATA_DIR / "final_backtest_fixed.json"
    if prev_path.exists():
        with open(prev_path) as f:
            prev = json.load(f)
        prev_ps = prev.get("portfolio_summary", {})
        print(f"\n{'=' * 60}")
        print("COMPARISON: R12 (125 bots) vs R13 (136 bots)")
        print(f"{'=' * 60}")
        print(f"  R12:  ${prev_ps.get('initial_wallet', 200):.0f} -> ${prev_ps.get('final_balance', 0):.2f}  "
              f"({prev_ps.get('total_return_pct', 0):+.1f}%)  DD {prev_ps.get('max_dd_pct', 0):.1f}%  "
              f"{prev_ps.get('total_trades', 0)} trades")
        print(f"  R13:  ${portfolio['initial_wallet']:.0f} -> ${portfolio['final_balance']:.2f}  "
              f"({portfolio['total_return_pct']:+.1f}%)  DD {portfolio['max_dd_pct']:.1f}%  "
              f"{portfolio['total_trades']} trades")

    # Print all 3 reports
    print(report_per_bot(per_bot_stats, bot_metrics, r13_labels))
    ...
    output = {
        "meta": {
            ...
            "r13_bots": [b.label for b in bots if b.is_r13],
            "r13_deployed": sorted(r13_labels),
    ...
    out_path = DATA_DIR / "r13_combined.json"
